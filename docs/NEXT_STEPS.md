...
4. Continue staged manual verification from `docs/manual-testing.md`, starting with visible framebuffer output, shell input, and ChronoFS basics.
...
- ChronoFS repair/journaling: implemented in code; `fsck`, `fsck repair`, and a tiny one-record journal exist, but recovery scenarios need runtime verification.
...
- Process model: partially implemented; ring 3, syscalls, and static ELF execution exist, but there is no dynamic linker, argv/env, or general multiprocess model.
...
3. Storage smoke pass: `ls`, `write`, `cat`, `rm`, `fsck`, `fsck repair`, and `journal`.
...
6. Risky systems last: custom BIOS path, UEFI path, SMP/AP startup, ring 3, syscalls, static ELF execution, and ARP/UDP networking.
