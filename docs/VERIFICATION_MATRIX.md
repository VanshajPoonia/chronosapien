...
| mouse/windowing | partially implemented | partially verified in QEMU | Serial logged `mouse: click at 740,410`; `open notes` spawned a task, logged `wm: open notes`, and produced a visible window boundary. | QEMU monitor mouse/key input; `/private/tmp/chronoos-ui-input-20260602-150049-open-notes-window.png`. | Cursor movement, drag, close, focus, and `open sysinfo` remain unverified. |
| apps | implemented in code | partially verified in QEMU | `apps` launcher, notes home screen, and `calc 6 - 7` result were observed. | QEMU monitor `sendkey`; screenshots `/private/tmp/chronoos-ui-input-20260602-150049-apps.png`, `...-notes-attempt.png`, and `...-calc.png`. | `sysinfo`, notes read/write, and persistence remain unverified. |
| ChronoFS | implemented in code | implemented in code, not verified | Shell commands and ATA-backed filesystem paths exist in source/docs. | Not runtime-tested in recorded QEMU passes. | `ls`, `write`, `cat`, `rm`, and persistence still need evidence. |
| fsck/journal | implemented in code | implemented in code, not verified | `fsck`, `fsck repair`, journal, and recovery paths exist in source/docs. | Not runtime-tested in recorded QEMU passes. | Repair/recovery should only be tested with controlled disk images. |
...
| Ring 3 | partially implemented | implemented in code, not verified | Ring 3 teaching path exists. | `ring3` was not run in recorded QEMU passes. | Do not imply general userland. |
| syscalls | partially implemented | implemented in code, not verified | Tiny syscall layer and `syshello` path exist. | `syshello` was not run in recorded QEMU passes. | Needs controlled userspace verification. |
| static ELF exec | partially implemented | implemented in code, not verified | Static ELF execution path exists. | `exec <name>` was not run in recorded QEMU passes. | No dynamic linker, package model, argv/env, or libc. |
| networking | partially implemented | partially verified in QEMU | RTL8139 discovery and MAC `52:54:00:12:34:56` were observed; host UDP log was 0 bytes. | Single-core QEMU with RTL8139; log `/private/tmp/chronoos-net-20260602-162000.serial.log`; host log `/private/tmp/chronoos-net-20260602-162000.host-udp.log`. | ARP/UDP behavior was not verified because host forwarding conflicted and `net` input garbled. |
...
| network demo | roadmap/design-only | roadmap/design-only | Current surface is `net`, `net arp`, and `net send`; no guided network demo mode exists. | Not applicable. | DHCP, DNS, TCP, and sockets remain roadmap/design-only. |
