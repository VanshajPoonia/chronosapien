...
6. Our kernel initializes a small COM1 serial logger, configures the VGA theme, loads the GDT and IDT, triggers one test breakpoint exception, remaps the PIC, starts the PIT timer, logs the boot sequence, prints the Time Capsule OS startup banner, and enters a polling keyboard input loop.
